...
curl -X POST -F "config=@$config_path" http://fontello.com 2>/dev/null > /data/.fontello_session_id
...
printf "You can manipulate the fonts on the browser at the URL: ${MAGENTA}http://fontello.com/$session_id\n${ORIGINAL}\n"
...
if [ "$input" == "y" ] || [ "$input" == "Y" ]; then
  wget -O fontello.zip "http://fontello.com/$session_id/get" 2> /dev/null
